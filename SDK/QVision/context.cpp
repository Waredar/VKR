...
void QtOpenGLFunctionList::glDebugMessageCallback(GLDEBUGPROC callback, const void* userParam) { 
    VSN_UNUSED(callback);
    VSN_UNUSED(userParam);
    /*m_pQtFunctions->glDebugMessageCallback(callback, userParam);*/
};
...
bool QtOpenGLContext::MakeCurrent()
{
    if (QOpenGLContext::makeCurrent(m_pWindow))
    {
        ...
        OpenGLContextInterface::SetCurrentContext(this);
        if (m_pFuncs == nullptr)
...
// ---
bool QtOpenGLContext::HasExtension(const String& extension) const
{ return QOpenGLContext::hasExtension(extension.c_str()); }
...
void QtOpenGLContextShell::DoneCurrent()
{
    ...
    OpenGLContextInterface::SetCurrentContext(nullptr);
}

OpenGLFunctionListInterface* QtOpenGLContextShell::GetFunctionList() const 
{ return GetFunctionListHelper(); }
...
bool QtOpenGLContextShell::HasExtension(const String& extension) const
{
    QByteArray extensionVal = extension.c_str();
    bool bResult = m_pWidget->context()->hasExtension(extensionVal);
    return bResult;
}
