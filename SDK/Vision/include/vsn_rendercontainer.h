...
using  RenderContainerIterator = ObjectMap<NodeKey, RenderObject*>::const_iterator;
//------------------------------------------------------------------------------
...
public:
    ...
    */
    RenderObject* GetObjectById(const NodeKey& objKey) const;
    ...
    */
    bool IsExistRenderObject(const NodeKey& objKey) const;
...
public:
    ...
    */
    ...
    /** \brief  \ru Удалить объект из контейнера.
        ...
        \return \ru True, если операция прошла успешно.
                \en True if operation was successfully done. \~
    */
    bool RemoveRenderObject(const NodeKey& objId);
    ...
    */
    ...
    /** \brief  \ru Снять выбор с объекта по идентификатору.
...
//    void DoRender(uint groupId, RenderMode renderingMode, Viewport* viewport = nullptr);
    ...
    /// \ru Вернуть итератор на первый элемент. \en Returns an iterator pointing to the first element in the list container.
    RenderContainerIterator Begin() const;
    /// \ru Вернуть итератор на элемент, следующий за последним. \en Returns an iterator referring to the past-the-end element in the list container..
    RenderContainerIterator End() const;
private:
    VSN_DECLARE_PRIVATE(RenderContainer)
};

typedef std::shared_ptr<RenderContainer> RenderContainerPtr;
