...
public:
    /** \brief \ru Специальный конструктор с представлением.
        ...
        \param[in] pRep - \ru Представление.
                          \en Representation. \~
    */
    explicit SceneSegmentData(GeometryRep* pRep = nullptr);
...
public:
    ...
    */
    ...
    const Matrix3DF& GetRelativeMatrix() const;
    ...
    */
    ...
    /// \ ru Изменить родителя для геометрии. \en Change parent for geometry. \~
...
public:
    ...
    */
    SceneSegmentData* Move(const Matrix3DF& matrix);
    ...
    */
    SceneSegmentData* SetMatrix(const Matrix3DF& matrix);

    /** \brief \ru Сбросить матрицу в единичную.
    ...
    */
    ...
    /// \ru Обновить абсолютную матрицу для всех подсегментов. \en Refreshes absolute matrix for all subsegments. \~
    void UpdateDataAbsoluteMatrix();
private:
    int* m_pDataCount = nullptr;                 /// количество вставок сегментов
    SceneSegmentRef* m_pDataReference = nullptr; /// указатель на ссылку сегмента
    ObjectList<SceneSegment*> m_segments; /// массив сегментов сцены
    Matrix3DF m_relativeMatrix;           /// относительная матрица
    String m_name;                        /// имя сигмента данных
};
