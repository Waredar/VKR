...
typedef ObjectVector<Feature*> FeatureVector;
//------------------------------------------------------------------------------
/** \brief   \ru Essence - подкласс узла, который может агрегировать несколько экземпляров компонентов для определения поведения сущности.
             \en Essence is a node subclass that aggregating several instances of features for entity behavior definition. \~
    \details \ru Сам по себе класс Essence является пустой оберткой. Поведение объекта Essence определяется объектами Feature,
             ...
             \en Essence class is an empty wrapper in itself. The behavior of Essence object is defined by Feature objects it refers to.
