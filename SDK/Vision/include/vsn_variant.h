...
public:
    ...
    /// \ru Конструктор с unsigned integer значением. \en Constructor with unsigned integer value. \~
    Variant(unsigned long value) : m_type(VAR_NONE) { *this = (unsigned long)value; }
    ...
    Variant(const RectI& value) : m_type(VAR_NONE) { *this = value; }
    /// \ru Конструктор с Matrix3DF значением. \en Constructor with Matrix3DF value. \~
    Variant(const Matrix3DF& value) : m_type(VAR_NONE) { *this = value; }
...
public:
    ...
    /// \ru Присвоить значение из unsigned long. \en Assigns value from unsigned long. \~
    Variant& operator =(unsigned long other) { SetType(VAR_UINT32); m_value.uint32_ = other; return *this; }
    ...
    Variant& operator =(const RectI& other) { SetType(VAR_INTRECT); *(reinterpret_cast<RectI*>(&m_value)) = other; return *this;}
    /// \ru Присвоить значение из Matrix3DF. \en Assigns value from Matrix3DF. \~
    Variant& operator =(const Matrix3DF& other) { SetType(VAR_MATRIX4); *(reinterpret_cast<Matrix3DF*>(m_value.ptr_)) = other; return *this; }
    ...
    /// \ru Проверить равенство с целым числом без знака. Вернет true, если есть совпадение по типу и значению.
    /// \en Checks equality with unsigned integer. Returns true if there is coincidence of type and value. \~
    bool operator ==(unsigned long other) const { return m_type == VAR_UINT32 ? m_value.uint32_ == other : false; }
    ...
    bool operator ==(const RectI& other) const { return m_type == VAR_INTRECT ? *(reinterpret_cast<const RectI*>(&m_value)) == other : false; }
    /// \ru Проверка на равенство с Matrix3DF. Вернет true, если есть совпадение по типу и значению.
    /// \en Checks equality with Matrix3DF. Returns true if there is coincidence of type and value. \~
    bool operator ==(const Matrix3DF& other) const { return m_type == VAR_MATRIX4 ? *(reinterpret_cast<const Matrix3DF*>(m_value.ptr_)) == other : false; }
    ...
    /// \ru Проверка неравенства с целым числом без знака. \en Checks inequality with unsigned integer. \~
    bool operator !=(unsigned long other) const { return !(*this == other); }
...
//    bool operator !=(const VariantMap& other) const { return !(*this == other); }
    ...
    /// \ru Проверка неравенства с Matrix3DF. \en Checks inequality with Matrix3DF. \~
    bool operator !=(const Matrix3DF& other) const { return !(*this == other); }
    ...
    int GetInt() const
    {
        if (m_type == VAR_INT)
            return m_value.int_;
        else if (m_type == VAR_UINT32)
            return (int)m_value.uint32_;
    ...
    unsigned GetUInt() const
    {
        if (m_type == VAR_INT)
            ...
            return (unsigned)m_value.uint32_;
        else if (m_type == VAR_FLOAT)
    ...
    const RectI& GetIntRect() const { return m_type == VAR_INTRECT ? *reinterpret_cast<const RectI*>(&m_value) : RectI::ZERO; }
    /// \ru Вернуть Matrix3DF или единичную матрицу, если тип не соответствует. \en Returns Matrix3DF or unitary matrix if type mismatch. \~
    const Matrix3DF& GetMatrix4() const { return m_type == VAR_MATRIX4 ? *(reinterpret_cast<const Matrix3DF*>(m_value.ptr_)) : Matrix3DF::identity; }
...
template <> inline VariantType GetVariantType<unsigned long>()             { return VAR_UINT32; }
...
template <> inline VariantType GetVariantType<RectI>()            { return VAR_INTRECT; }
template <> inline VariantType GetVariantType<Matrix3DF>()         { return VAR_MATRIX4; }

// Специализация для Variant::Get<T>
template <> int                     Variant::Get<int>() const;
template <> unsigned                Variant::Get<unsigned>() const;
template <> unsigned long                  Variant::Get<unsigned long>() const;
...
template <> const RectI&            Variant::Get<const RectI&>() const;
template <> void*                   Variant::Get<void*>() const;
template <> MbItem*                 Variant::Get<MbItem*>() const;
template <> const Matrix3DF&        Variant::Get<const Matrix3DF&>() const;
...
template <> RectI                   Variant::Get<RectI>() const;
template <> Matrix3DF               Variant::Get<Matrix3DF>() const;
