...
protected:
    ...
    explicit SceneModification(ModificationFlag type, const NodeKey& objectKey);
    /// \ru Конструктор с типом и уникальным ключом. \en Constructor with type and unique key.
    explicit SceneModification(SceneModificationPrivate &dd, ModificationFlag type, const NodeKey& objectKey);
protected:
