...
*/
// ---
class VSN_CLASS OpenGLContextContainer : public Object
{
    VSN_OBJECT(OpenGLContextContainer)
public:
...
public:
    ...
    ObjectList<OpenGLContextInterface*> GetContextList() const;
...
private:
    VSN_DECLARE_EX_PRIVATE(OpenGLContextContainer)
};
...
public:
    ...
    /// \ru Возвращает список функций текущего контекста. \en Returns a function list of the current context.
    static OpenGLFunctionListInterface* Gl();
    static bool ShouldCreateDebugContext();
public:
...
public:
    ...
    virtual void oglLoadMatrix(const Matrix3DF& matrix);
    virtual void oglMultMatrix(const Matrix3DF& matrix);
    virtual void oglOrtho(float left, float right, float bottom, float top, float nearVal, float farVal);
    virtual void oglFrustum(float left, float right, float bottom, float top, float nearVal, float farVal);
...
public:
protected:
    static void SetCurrentContext(OpenGLContextInterface*);
    OpenGLFunctionListInterface* GetFunctionListHelper() const;
    void SetFunctionList(OpenGLFunctionListInterface* funcList);
//private:
public:
    static OpenGLContextInterface* m_pCurrentContext;
protected:
