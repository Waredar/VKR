...
public:
    ...
    inline bool operator>=(String str) const { return !operator<(str); }
    ...
    void Remove(char c);
    /// \ru Удаляет n символов с конца строки. \en Removes n characters from the end of the string.
    ...
    ConstIterator CBegin() const { return ConstIterator(m_buffer); }
    ConstIterator cbegin() const { return ConstIterator(m_buffer); }
    ...
    ConstIterator CEnd() const { return ConstIterator(m_buffer + m_length); }
    ConstIterator cend() const { return ConstIterator(m_buffer + m_length); }
