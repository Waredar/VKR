...
public:
    ...
    ExpectedSlot(ExpectedSlot&&) = default;
...
private: // Slot
    ...
    virtual void AsyncSlot(ExpectedSlot data, ConnectionInner type);
    void DestroySlots();
public:
...
private:
    static Mutex mutex_beingDestroyed;
};
...
// ---
template<class Sender, class Receiver>
bool PrivateDisconnect(const Sender& sender, const Internal::AbsStorageFunction* pStorageSignal, 
    const Receiver* pReceiver, const Internal::AbsStorageFunction* pStorageSlot)
{
    bool retval = false;
    bool isDone = false;
    MutexLocker senderLock { &sender.m_mutexConnectList };
