...
*/
// ---
class VSN_CLASS OpenGLFramebufferObject final
{
public:
...
public:
    ...
    void Bind();
    /// \ru Отвязать Framebuffer. \en Releases Framebuffer. \~
    void Release();
    ...
    /// \ru Перекладывает содержимое буфера в текстуру глубины (текстуру удалять не нужно). \en Makes depth texture from buffer content. Texture doesn't need to be deleted. \~
